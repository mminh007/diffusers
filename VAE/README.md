...
$$p_\theta(x) = \int_{z}p_\theta(x,z)dz = \int_{z} p_\theta(z) p_\theta(x|z)dz (1)$$
...
$$ \rightarrow log p_\theta(x) - \mathbb{E}_{q_\phi(z|x)}[log \frac{p_\theta(x,z)}{q_\phi(z|x)}] = D_{KL} (q_\phi(z|x) || p_\theta(z|x)) \quad (**)$$
...
$$\frac1{k} \sum^{k}_{i=1} log p_\theta(x, z^i) q_\phi(z^i) \quad with \quad z^i \thicksim q_\phi(z|x) \quad (***) $$
...
$$ \max_\theta \sum_{x \in \mathcal{D}} \max_\phi \mathbb{E}_{q_\phi(z|x)} [log \frac{p_\theta(x,z)}{q_\phi(z|x)}] $$ 
...
$$ \mathcal{L}_{(\theta, \phi)}(x) := \mathbb{E}_{q_\phi(z|x)}[log \frac{p_\theta(x,z)}{q_\phi(z|x)}] $$  
...
$$ = \mathbb{E}_{q_\phi(z|x)}[log \frac{p_\theta(x|z) p_\theta(z)}{q_\phi(z|x)}] $$

$$ = \mathbb{E}_{q_\phi(z|x)}[log (p_\theta(x|z)) + log \frac{p_\theta(z)}{q_\phi(z|x)}] $$

$$ = \mathbb{E}_{q_\phi(z|x)}[log (p_\theta(x|z)) - log \frac{q_\phi(z|x)}{p_\theta(z)}] $$

$$ = \mathbb{E}_{q_\phi(z|x)} [log (p_\theta(x|z))] - \mathbb{E}_{q_\phi(z|x)} [log \frac{q_\phi(z|x)}{p_\theta(z)}] $$

-   The **first** term: $$\mathcal{L}_{E} = \mathbb{E}_{q_\phi(z|x)} log (p_\theta(x|z))$$ represents the **reconstruction loss**, which measures how well the decoder can reconstruct the input data.
  
- The **second** term: $$ \mathcal{L}_{R} = \mathbb{E}_{q_\phi(z|x)} [log \frac{q_\phi(z|x)}{p_\theta(z)}]$$ represents the **KL regularization term**, which ensures that the approximate posterior $q_{\phi}(z|x)$ does not deviate too much from the prior $p_{\theta}(z)$.
...
$$ \nabla_\theta \mathbb{E}_{q_\phi(z|x)}[log \frac{p_\theta(x,z)}{q_\phi(z|x)}] = \mathbb{E}_{q_\phi(z|x)} [\nabla_\theta log \frac{p_\theta(x,z)}{q_\phi(z|x)}] $$
...
$$ \mu_\phi(x) + \mathbb{L}_\phi(x) \cdot \epsilon $$

where $\mathbb{L}_\phi(x)$ is the Cholesky decomposition of the covariance matrix:

$$ \Sigma_{\phi}(x)) = \mathbb{L}_\phi(x) \mathbb{L}_\phi(x)^T $$

:bulb: **Computing the Gradient $\nabla_\phi ELBO$:**

Using the reparameterization trick, the gradient can now be rewritten as:

$$ \nabla_\phi \mathbb{E}_{q_\phi(z|x)}[log \frac{p_\theta(x,z)}{q_\phi(z|x)}] = \mathbb{E}_\epsilon [\nabla_\phi log \frac{p_\theta(x, \mu_\phi(x)+\mathbb{L}_\phi(x)\cdot \epsilon)}{q_\phi(\mu_\phi(x) + \mathbb{L}_\phi(x)|x)}] $$
...
- $x^{i}_{d_{\theta}(z)})^2$  is the reconstructed output from the decoder.
